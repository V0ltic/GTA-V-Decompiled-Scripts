...
void __EntryFunction__()
{
	...
	while (true)
	{
		...
		if (func_23(player_id()) && does_entity_exist(func_22()))
		{
			iLocal_31 = func_22();
		}
		else
		{
			iLocal_31 = player_ped_id();
		}
		if (is_entity_dead(iLocal_31, false))
		{
		}
		func_1();
	}
...
void func_1()
{
	...
	{
		Var0 = { get_entity_coords(iLocal_31, false) };
		...
		if (vdist2(Var1, Var0) < IntToFloat(func_18(iLocal_29)))
		{
			...
			while (iVar3 < func_17(iLocal_29))
			{
				if (!bVar2)
				{
					if (is_entity_in_angled_area(iLocal_31, func_16(iLocal_29, iVar3), func_15(iLocal_29, iVar3), func_14(iLocal_29, iVar3), false, true, 0))
					{
						bVar2 = true;
					}
				}
			...
			if (bVar2)
			{
				set_radar_as_interior_this_frame(get_hash_key(func_13(iLocal_29)), Var1.f_0, Var1.f_1, func_12(iLocal_29), func_11(iLocal_29));
				func_10(iLocal_29);
			}
		}
	}
	if (network_is_game_in_progress())
	{
		if (func_2())
		{
			...
			{
				clear_bit(&uLocal_32, 1);
			}
			if (is_pause_menu_active())
			{
				if (!BitTest(uLocal_32, 0))
				{
					set_minimap_component(15, true, -1);
					set_bit(&uLocal_32, 0);
				}
			}
			else if (BitTest(uLocal_32, 0))
			{
				set_minimap_component(15, false, -1);
				clear_bit(&uLocal_32, 0);
			}
		}
