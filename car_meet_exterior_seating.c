#region Local Var
	var uLocal_0 = 0;
	var uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	var uLocal_12 = 0;
	var uLocal_13 = 0;
	float fLocal_14 = 0f;
	var uLocal_15 = 0;
	var uLocal_16 = 0;
	int iLocal_17 = 0;
	char* sLocal_18 = NULL;
	var uLocal_19 = 0;
	var uLocal_20 = 0;
	float fLocal_21 = 0f;
	var uLocal_22 = 0;
	var uLocal_23 = 0;
	var uLocal_24 = 0;
	float fLocal_25 = 0f;
	float fLocal_26 = 0f;
	var uLocal_27 = 0;
	int iLocal_28 = 0;
	var uLocal_29 = 0;
	var uLocal_30 = 0;
	float fLocal_31 = 0f;
	float fLocal_32 = 0f;
	float fLocal_33 = 0f;
	var uLocal_34 = 0;
	var uLocal_35 = 0;
	var uLocal_36 = 0;
	var uLocal_37 = 0;
	var uLocal_38 = 0;
	int iLocal_39 = 0;
	int iLocal_40 = 0;
	int iLocal_41 = 0;
	int iLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	var uLocal_47 = 0;
	var uLocal_48 = 0;
	var uLocal_49 = 0;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 0;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	var uLocal_61 = 0;
	float fLocal_62 = 0f;
	var uLocal_63 = 0;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = 0;
	var uLocal_68 = 0;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	var uLocal_71 = 0;
	var uLocal_72 = 0;
	var uLocal_73 = 0;
	var uLocal_74 = 0;
	var uLocal_75 = 0;
	var uLocal_76 = 0;
	var uLocal_77 = 0;
	var uLocal_78 = 0;
	var uLocal_79 = 0;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = -1;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 0;
	var uLocal_89 = 0;
	var uLocal_90 = 0;
	var uLocal_91 = 0;
	var uLocal_92 = 0;
	var uLocal_93 = 0;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	var uLocal_97 = 0;
	var uLocal_98 = 0;
	var uLocal_99 = -1;
	var uLocal_100 = 0;
	var uLocal_101 = -1;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = -1;
	var uLocal_105 = -1;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 0;
	var uLocal_109 = 15;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 1061158912;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 0;
	var uLocal_124 = 0;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 1061158912;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	var uLocal_135 = 0;
	var uLocal_136 = 0;
	var uLocal_137 = 0;
	var uLocal_138 = 0;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 1061158912;
	var uLocal_145 = 0;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	var uLocal_148 = 0;
	var uLocal_149 = 0;
	var uLocal_150 = 0;
	var uLocal_151 = 0;
	var uLocal_152 = 0;
	var uLocal_153 = 0;
	var uLocal_154 = 0;
	var uLocal_155 = 0;
	var uLocal_156 = 0;
	var uLocal_157 = 0;
	var uLocal_158 = 1061158912;
	var uLocal_159 = 0;
	var uLocal_160 = 0;
	var uLocal_161 = 0;
	var uLocal_162 = 0;
	var uLocal_163 = 0;
	var uLocal_164 = 0;
	var uLocal_165 = 0;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 0;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 1061158912;
	var uLocal_173 = 0;
	var uLocal_174 = 0;
	var uLocal_175 = 0;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 0;
	var uLocal_184 = 0;
	var uLocal_185 = 0;
	var uLocal_186 = 1061158912;
	var uLocal_187 = 0;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 0;
	var uLocal_194 = 0;
	var uLocal_195 = 0;
	var uLocal_196 = 0;
	var uLocal_197 = 0;
	var uLocal_198 = 0;
	var uLocal_199 = 0;
	var uLocal_200 = 1061158912;
	var uLocal_201 = 0;
	var uLocal_202 = 0;
	var uLocal_203 = 0;
	var uLocal_204 = 0;
	var uLocal_205 = 0;
	var uLocal_206 = 0;
	var uLocal_207 = 0;
	var uLocal_208 = 0;
	var uLocal_209 = 0;
	var uLocal_210 = 0;
	var uLocal_211 = 0;
	var uLocal_212 = 0;
	var uLocal_213 = 0;
	var uLocal_214 = 1061158912;
	var uLocal_215 = 0;
	var uLocal_216 = 0;
	var uLocal_217 = 0;
	var uLocal_218 = 0;
	var uLocal_219 = 0;
	var uLocal_220 = 0;
	var uLocal_221 = 0;
	var uLocal_222 = 0;
	var uLocal_223 = 0;
	var uLocal_224 = 0;
	var uLocal_225 = 0;
	var uLocal_226 = 0;
	var uLocal_227 = 0;
	var uLocal_228 = 1061158912;
	var uLocal_229 = 0;
	var uLocal_230 = 0;
	var uLocal_231 = 0;
	var uLocal_232 = 0;
	var uLocal_233 = 0;
	var uLocal_234 = 0;
	var uLocal_235 = 0;
	var uLocal_236 = 0;
	var uLocal_237 = 0;
	var uLocal_238 = 0;
	var uLocal_239 = 0;
	var uLocal_240 = 0;
	var uLocal_241 = 0;
	var uLocal_242 = 1061158912;
	var uLocal_243 = 0;
	var uLocal_244 = 0;
	var uLocal_245 = 0;
	var uLocal_246 = 0;
	var uLocal_247 = 0;
	var uLocal_248 = 0;
	var uLocal_249 = 0;
	var uLocal_250 = 0;
	var uLocal_251 = 0;
	var uLocal_252 = 0;
	var uLocal_253 = 0;
	var uLocal_254 = 0;
	var uLocal_255 = 0;
	var uLocal_256 = 1061158912;
	var uLocal_257 = 0;
	var uLocal_258 = 0;
	var uLocal_259 = 0;
	var uLocal_260 = 0;
	var uLocal_261 = 0;
	var uLocal_262 = 0;
	var uLocal_263 = 0;
	var uLocal_264 = 0;
	var uLocal_265 = 0;
	var uLocal_266 = 0;
	var uLocal_267 = 0;
	var uLocal_268 = 0;
	var uLocal_269 = 0;
	var uLocal_270 = 1061158912;
	var uLocal_271 = 0;
	var uLocal_272 = 0;
	var uLocal_273 = 0;
	var uLocal_274 = 0;
	var uLocal_275 = 0;
	var uLocal_276 = 0;
	var uLocal_277 = 0;
	var uLocal_278 = 0;
	var uLocal_279 = 0;
	var uLocal_280 = 0;
	var uLocal_281 = 0;
	var uLocal_282 = 0;
	var uLocal_283 = 0;
	var uLocal_284 = 1061158912;
	var uLocal_285 = 0;
	var uLocal_286 = 0;
	var uLocal_287 = 0;
	var uLocal_288 = 0;
	var uLocal_289 = 0;
	var uLocal_290 = 0;
	var uLocal_291 = 0;
	var uLocal_292 = 0;
	var uLocal_293 = 0;
	var uLocal_294 = 0;
	var uLocal_295 = 0;
	var uLocal_296 = 0;
	var uLocal_297 = 0;
	var uLocal_298 = 1061158912;
	var uLocal_299 = 0;
	var uLocal_300 = 0;
	var uLocal_301 = 0;
	var uLocal_302 = 0;
	var uLocal_303 = 0;
	var uLocal_304 = 0;
	var uLocal_305 = 0;
	var uLocal_306 = 0;
	var uLocal_307 = 0;
	var uLocal_308 = 0;
	var uLocal_309 = 0;
	var uLocal_310 = 0;
	var uLocal_311 = 0;
	var uLocal_312 = 1061158912;
	var uLocal_313 = 0;
	var uLocal_314 = 0;
	var uLocal_315 = 0;
	var uLocal_316 = 0;
	var uLocal_317 = 0;
	var uLocal_318 = 0;
	var uLocal_319 = 0;
#endregion

void __EntryFunction__()
{
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_14 = 0.001f;
	iLocal_17 = -1;
	sLocal_18 = "NULL";
	fLocal_21 = 0f;
	fLocal_25 = -0.0375f;
	fLocal_26 = 0.17f;
	iLocal_28 = 3;
	fLocal_31 = 80f;
	fLocal_32 = 140f;
	fLocal_33 = 180f;
	iLocal_39 = 1;
	iLocal_40 = 65;
	iLocal_41 = 49;
	iLocal_42 = 64;
	fLocal_62 = ((0.05f + 0.275f) - 0.01f);
	if (network_is_game_in_progress())
	{
		func_79();
	}
	else
	{
		func_76();
	}
	while (true)
	{
		wait(0);
		if (func_64())
		{
			func_76();
		}
		func_8(&uLocal_93);
		func_1(&uLocal_93);
	}
}

void func_1(var uParam0)
{
	int iVar0;
	
	if (((is_ped_injured(player_ped_id()) || is_ped_ragdoll(player_ped_id())) || is_ped_in_combat(player_ped_id(), 0)) || is_ped_in_melee_combat(player_ped_id()))
	{
		func_6(&(uParam0->f_6));
		func_4(uParam0, 1);
		if (uParam0->f_13 > 2)
		{
			func_3(-1);
			iVar0 = network_get_local_scene_from_network_id(uParam0->f_8);
			if (is_synchronized_scene_running(iVar0))
			{
				network_stop_synchronised_scene(uParam0->f_8);
			}
		}
		func_2(uParam0, 0);
	}
}

void func_2(var uParam0, int iParam1)
{
	uParam0->f_13 = iParam1;
}

void func_3(int iParam0)
{
	Global_2725432 = iParam0;
	Global_1853348[player_id() /*834*/].f_827 = iParam0;
}

void func_4(var uParam0, int iParam1)
{
	if (((func_5("MPOF_AC_PC_EXIT") || func_5("MPOF_AC_EXIT")) || func_5("MPOFSEAT_PCEXIT")) || func_5("MPOFSEAT_EXIT"))
	{
		clear_help(true);
	}
	if (iParam1 && uParam0->f_6 != -1)
	{
		func_6(&(uParam0->f_6));
		uParam0->f_6 = -1;
	}
}

bool func_5(char* sParam0)
{
	begin_text_command_is_this_help_message_being_displayed(sParam0);
	return end_text_command_is_this_help_message_being_displayed(0);
}

void func_6(var uParam0)
{
	int iVar0;
	
	if (*uParam0 == -1)
	{
		return;
	}
	iVar0 = func_7(*uParam0);
	if (iVar0 == -1)
	{
		*uParam0 = -1;
		return;
	}
	if (iVar0 > -1 && iVar0 < 6)
	{
		if (Global_43792[iVar0 /*32*/])
		{
			Global_43792[iVar0 /*32*/].f_7 = 1;
			*uParam0 = -1;
			return;
		}
	}
	*uParam0 = -1;
}

int func_7(int iParam0)
{
	int iVar0;
	
	if (iParam0 < 0)
	{
		return -1;
	}
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 6)
	{
		if (Global_43792[iVar0 /*32*/].f_1 == iParam0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

void func_8(var uParam0)
{
	char* sVar0;
	char* sVar1;
	struct<3> Var2;
	struct<3> Var3;
	float fVar4;
	float fVar5;
	int iVar6;
	struct<3> Var7;
	float fVar8;
	int iVar9;
	int iVar10;
	int iVar11;
	
	func_61(uParam0);
	func_59(uParam0);
	switch (uParam0->f_13)
	{
		case 0:
			func_58(&(uParam0->f_7), 11);
			func_2(uParam0, 1);
			break;
		
		case 1:
			Global_1946234 = 0;
			if (func_57(uParam0))
			{
				uParam0->f_11 = get_game_timer();
				func_2(uParam0, 2);
			}
			break;
		
		case 2:
			if (func_56(player_ped_id(), &(uParam0->f_16[uParam0->f_9 /*14*/])) && func_55(player_ped_id(), uParam0->f_16[uParam0->f_9 /*14*/].f_3, uParam0->f_16[uParam0->f_9 /*14*/].f_13))
			{
				if (((((((((((func_54(uParam0) || func_50(uParam0)) || func_49(uParam0)) || !func_48(uParam0)) || func_46(player_id())) || is_ped_in_any_vehicle(player_ped_id(), true)) || is_ped_in_combat(player_ped_id(), 0)) || is_ped_in_melee_combat(player_ped_id())) || is_ped_performing_melee_action(player_ped_id())) || is_ped_ragdoll(player_ped_id())) || is_ped_injured(player_ped_id())) || BitTest(Global_1946250.f_4, 2))
				{
					if (func_50(uParam0))
					{
						if (uParam0->f_6 == -1)
						{
							func_45(&(uParam0->f_6), 4, "POD_TOO_MANY", 0, 0, 0, 0);
						}
					}
					else
					{
						func_6(&(uParam0->f_6));
						func_4(uParam0, 1);
						func_2(uParam0, 1);
					}
				}
				else
				{
					Global_1946234 = 1;
					if (uParam0->f_6 == -1)
					{
						if ((get_game_timer() - uParam0->f_11) > 150)
						{
							func_45(&(uParam0->f_6), 4, "MPJAC_SIT", 0, 0, 0, 0);
						}
					}
					else if (is_control_just_pressed(0, 51))
					{
						func_6(&(uParam0->f_6));
						func_42(uParam0);
						func_2(uParam0, 3);
					}
				}
			}
			else
			{
				func_4(uParam0, 1);
				func_2(uParam0, 1);
			}
			break;
		
		case 3:
			func_41(uParam0, &sVar0);
			request_anim_dict(&sVar0);
			if (has_anim_dict_loaded(&sVar0))
			{
				func_2(uParam0, 4);
			}
			break;
		
		case 4:
			func_41(uParam0, &sVar0);
			func_40(uParam0, &sVar1);
			disable_control_action(0, 0, true);
			Var2 = { get_anim_initial_offset_position(&sVar0, &sVar1, func_39(uParam0), func_38(uParam0), 0f, 2) };
			Var3 = { get_anim_initial_offset_rotation(&sVar0, &sVar1, func_39(uParam0), func_38(uParam0), 0f, 2) };
			fVar4 = Var3.f_2;
			fVar5 = 0.05f;
			if (uParam0->f_1.f_1 == 1)
			{
				fVar5 = 0.15f;
			}
			task_go_straight_to_coord(player_ped_id(), Var2, 1f, 20000, fVar4, fVar5);
			func_3(uParam0->f_9);
			func_2(uParam0, 5);
			break;
		
		case 5:
			func_26(uParam0);
			func_41(uParam0, &sVar0);
			func_40(uParam0, &sVar1);
			iVar6 = get_script_task_status(player_ped_id(), 2106541073);
			Var7 = { get_anim_initial_offset_rotation(&sVar0, &sVar1, func_39(uParam0), func_38(uParam0), 0f, 2) };
			fVar8 = Var7.f_2;
			if ((iVar6 != 1 && iVar6 != 0) || func_25(get_entity_heading(player_ped_id()), fVar8, 2f))
			{
				disable_control_action(0, 0, true);
				uParam0->f_8 = network_create_synchronised_scene(func_39(uParam0), func_38(uParam0), 2, true, false, 1f, 0f, 1.12f);
				network_add_ped_to_synchronised_scene(player_ped_id(), uParam0->f_8, &sVar0, &sVar1, 2f, -1000f, 9, 0, 1000f, 0);
				network_start_synchronised_scene(uParam0->f_8);
				uParam0->f_1 = get_game_timer();
				func_24(&(uParam0->f_7), 8);
				func_2(uParam0, 6);
			}
			break;
		
		case 6:
			if (func_23(&(uParam0->f_7), 10) || func_22())
			{
				func_58(&(uParam0->f_7), 10);
				func_21(uParam0);
			}
			func_26(uParam0);
			func_15(uParam0);
			iVar9 = network_get_local_scene_from_network_id(uParam0->f_8);
			if (func_23(&(uParam0->f_7), 13))
			{
				disable_control_action(0, 0, true);
			}
			if (is_synchronized_scene_running(iVar9))
			{
				if (get_synchronized_scene_phase(iVar9) >= 1f)
				{
					if (func_23(&(uParam0->f_7), 13))
					{
						if (get_cam_view_mode_for_context(0) == 1)
						{
							set_cam_view_mode_for_context(0, 4);
						}
						func_58(&(uParam0->f_7), 13);
					}
					func_9(uParam0, 0);
					func_41(uParam0, &sVar0);
					func_40(uParam0, &sVar1);
					uParam0->f_8 = network_create_synchronised_scene(func_39(uParam0), func_38(uParam0), 2, true, false, 1f, 0f, 1.12f);
					network_add_ped_to_synchronised_scene(player_ped_id(), uParam0->f_8, &sVar0, &sVar1, 4f, -1.5f, 9, 0, 1000f, 0);
					network_start_synchronised_scene(uParam0->f_8);
					func_24(&(uParam0->f_7), 8);
					func_24(&(uParam0->f_7), 11);
				}
				else
				{
					func_58(&(uParam0->f_7), 8);
				}
			}
			else if (!func_23(&(uParam0->f_7), 8))
			{
				func_2(uParam0, 8);
			}
			break;
		
		case 7:
			network_stop_synchronised_scene(uParam0->f_8);
			func_2(uParam0, 8);
			break;
		
		case 8:
			iVar10 = network_get_local_scene_from_network_id(uParam0->f_8);
			iVar11 = get_script_task_status(player_ped_id(), 1785177548);
			if (iVar11 == 0 || iVar11 == 1)
			{
				if (is_synchronized_scene_running(iVar10))
				{
					if ((get_synchronized_scene_phase(iVar10) >= 0.7f || has_anim_event_fired(player_ped_id(), get_hash_key("BREAKOUT_FINISH"))) || has_anim_event_fired(player_ped_id(), get_hash_key("BREAK_OUT")))
					{
						network_stop_synchronised_scene(uParam0->f_8);
						func_3(-1);
						func_4(uParam0, 1);
						func_2(uParam0, 0);
					}
				}
			}
			else
			{
				func_3(-1);
				func_4(uParam0, 1);
				func_2(uParam0, 0);
			}
			break;
	}
}

void func_9(var uParam0, bool bParam1)
{
	int iVar0;
	int iVar1;
	int iVar2;
	
	if (bParam1)
	{
		iVar1 = func_10(uParam0);
		if (iVar1 != uParam0->f_1.f_2)
		{
			if (iVar1 == 0)
			{
				iVar0 = (11 + uParam0->f_1.f_2);
			}
			else
			{
				iVar0 = (6 + iVar1);
			}
		}
		else
		{
			iVar0 = uParam0->f_1.f_3;
		}
		iVar1 = uParam0->f_1.f_2;
	}
	else if (uParam0->f_1.f_3 == 5)
	{
		iVar1 = uParam0->f_1.f_2;
		iVar2 = 3;
		iVar0 = get_random_int_in_range(0, iVar2);
		if (iVar0 == uParam0->f_1.f_4)
		{
			iVar0 = (iVar0 + 1 % iVar2);
		}
	}
	else
	{
		if (uParam0->f_1.f_3 > 6)
		{
			iVar1 = func_10(uParam0);
			uParam0->f_1 = get_game_timer();
		}
		else
		{
			iVar1 = uParam0->f_1.f_2;
		}
		if ((get_game_timer() - uParam0->f_1) >= 90000)
		{
			iVar1 = func_10(uParam0);
			if (iVar1 != uParam0->f_1.f_2)
			{
				if (iVar1 == 0)
				{
					iVar0 = (11 + uParam0->f_1.f_2);
				}
				else
				{
					iVar0 = (6 + iVar1);
				}
			}
			iVar1 = uParam0->f_1.f_2;
		}
		else
		{
			iVar0 = 5;
		}
	}
	uParam0->f_1.f_4 = uParam0->f_1.f_3;
	uParam0->f_1.f_3 = iVar0;
	uParam0->f_1.f_2 = iVar1;
}

int func_10(var uParam0)
{
	int iVar0;
	
	if (uParam0->f_1.f_1 == 2)
	{
		return uParam0->f_1.f_2;
	}
	iVar0 = (uParam0->f_1.f_2 + 1 % func_13(uParam0->f_1.f_1));
	if (func_11())
	{
		if (uParam0->f_1.f_1 == 1)
		{
			if (iVar0 == 0 || iVar0 == 4)
			{
				iVar0 = 1;
			}
		}
	}
	else if (uParam0->f_1.f_1 == 1)
	{
		if (iVar0 == 1 || iVar0 == 2)
		{
			iVar0 = 3;
		}
	}
	return iVar0;
}

bool func_11()
{
	return func_12(player_id());
}

int func_12(int iParam0)
{
	if (get_entity_model(get_player_ped(iParam0)) == joaat("mp_f_freemode_01"))
	{
		return 1;
	}
	return 0;
}

int func_13(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 3;
		
		case 1:
			return func_14(func_11(), 4, 5);
		
		case 2:
			return 3;
		
		default:
	}
	return 0;
}

int func_14(bool bParam0, int iParam1, int iParam2)
{
	if (bParam0)
	{
		return iParam1;
	}
	return iParam2;
}

void func_15(var uParam0)
{
	struct<3> Var0;
	float fVar1;
	int iVar2;
	
	if (uParam0->f_13 == 6)
	{
		Var0 = { get_control_normal(0, 218), get_control_normal(0, 219), 0f };
		fVar1 = vmag(Var0);
		if ((uParam0->f_13 == 6 && !has_anim_event_fired(player_ped_id(), get_hash_key("BLOCK_INTERRUPT"))) && !func_23(&(uParam0->f_7), 12))
		{
			if ((fVar1 >= 0.35f && !Global_2789734) && !Global_2789737)
			{
				func_20(uParam0);
			}
		}
		if (_0x6CD79468A1E595C6(2))
		{
			func_4(uParam0, 1);
		}
		if (has_anim_event_fired(player_ped_id(), get_hash_key("BLOCK_INTERRUPT")))
		{
			func_4(uParam0, 1);
		}
		else if (Global_2789734 || Global_2789737)
		{
			func_4(uParam0, 1);
		}
		else if (func_19(uParam0))
		{
			func_4(uParam0, 1);
		}
		else
		{
			_0x7F4724035FDCA1DD(2);
			disable_control_action(0, 24, true);
			disable_control_action(0, 257, true);
			disable_control_action(0, 142, true);
			disable_control_action(0, 141, true);
			disable_control_action(0, 140, true);
			disable_control_action(0, 263, true);
			disable_control_action(0, 264, true);
			disable_control_action(0, 143, true);
			if (((((!is_pause_menu_active() && !func_23(&(uParam0->f_7), 0)) && !func_23(&(uParam0->f_7), 4)) && !func_18()) && !func_16(1)) && uParam0->f_1.f_3 != 3)
			{
				if (uParam0->f_6 == -1)
				{
					if (uParam0->f_1.f_1 == 1 || uParam0->f_1.f_1 == 0)
					{
						if (_is_using_keyboard(2))
						{
							func_45(&(uParam0->f_6), 4, "MPOF_AC_PC_EXIT", 0, 0, 0, 0);
						}
						else
						{
							func_45(&(uParam0->f_6), 4, "MPOF_AC_EXIT", 0, 0, 0, 0);
						}
					}
					else if (_is_using_keyboard(2))
					{
						func_45(&(uParam0->f_6), 4, "MPOFSEAT_PCEXIT", 0, 0, 0, 0);
					}
					else
					{
						func_45(&(uParam0->f_6), 4, "MPOFSEAT_EXIT", 0, 0, 0, 0);
					}
				}
				if (uParam0->f_13 == 6)
				{
					if (_is_using_keyboard(2))
					{
						iVar2 = 225;
					}
					else
					{
						iVar2 = 190;
					}
					if (is_disabled_control_just_pressed(2, iVar2))
					{
						func_21(uParam0);
					}
				}
			}
		}
	}
	func_58(&(uParam0->f_7), 9);
}

int func_16(bool bParam0)
{
	if (get_allow_movement_while_zoomed())
	{
		if (!is_ped_injured(player_ped_id()))
		{
			if (func_17(player_ped_id()))
			{
				if (is_control_pressed(0, 25) || is_control_pressed(0, 68))
				{
					return 0;
				}
			}
		}
	}
	if (Global_23011.f_130)
	{
		return 0;
	}
	if (is_control_pressed(0, 19) || (!bParam0 && is_disabled_control_pressed(0, 19)))
	{
		return 1;
	}
	if (is_pc_version())
	{
		if (((is_control_pressed(0, 166) || is_control_pressed(0, 167)) || is_control_pressed(0, 168)) || is_control_pressed(0, 169))
		{
			return 1;
		}
		if (!bParam0)
		{
			if (((is_disabled_control_pressed(0, 166) || is_disabled_control_pressed(0, 167)) || is_disabled_control_pressed(0, 168)) || is_disabled_control_pressed(0, 169))
			{
				return 1;
			}
		}
	}
	return 0;
}

int func_17(int iParam0)
{
	int iVar0;
	
	if (is_first_person_aim_cam_active())
	{
		if (!is_ped_injured(iParam0))
		{
			get_current_ped_weapon(iParam0, &iVar0, true);
			if (((iVar0 == joaat("weapon_sniperrifle") || iVar0 == joaat("weapon_heavysniper")) || iVar0 == joaat("weapon_marksmanrifle")) || iVar0 == joaat("weapon_heavysniper_mk2"))
			{
				return 1;
			}
		}
	}
	return 0;
}

int func_18()
{
	if (Global_2703735.f_833.f_5 == -1)
	{
		return 0;
	}
	return 1;
}

bool func_19(var uParam0)
{
	return func_23(&(uParam0->f_7), 9);
}

void func_20(var uParam0)
{
	char* sVar0;
	char* sVar1;
	
	if (((func_10(uParam0) != uParam0->f_1.f_2 && uParam0->f_1.f_3 < 6) && uParam0->f_1.f_3 != 3) && uParam0->f_1.f_3 != 4)
	{
		func_9(uParam0, 1);
		func_41(uParam0, &sVar0);
		func_40(uParam0, &sVar1);
		uParam0->f_8 = network_create_synchronised_scene(func_39(uParam0), func_38(uParam0), 2, true, false, 1f, 0f, 1.12f);
		network_add_ped_to_synchronised_scene(player_ped_id(), uParam0->f_8, &sVar0, &sVar1, 4f, -1.5f, 9, 0, 1000f, 0);
		network_start_synchronised_scene(uParam0->f_8);
		func_24(&(uParam0->f_7), 8);
		force_ped_ai_and_animation_update(player_ped_id(), false, false);
	}
}

void func_21(var uParam0)
{
	char* sVar0;
	
	uParam0->f_8 = network_create_synchronised_scene(func_39(uParam0), func_38(uParam0), 2, true, false, 1f, 0f, 1.12f);
	func_41(uParam0, &sVar0);
	network_add_ped_to_synchronised_scene(player_ped_id(), uParam0->f_8, &sVar0, "exit", 4f, -4f, 9, 0, 1000f, 0);
	network_start_synchronised_scene(uParam0->f_8);
	func_6(&(uParam0->f_6));
	func_2(uParam0, 8);
}

var func_22()
{
	return BitTest(Global_1946250.f_3, 25);
}

bool func_23(var uParam0, int iParam1)
{
	return BitTest(*uParam0, iParam1);
}

void func_24(int* iParam0, int iParam1)
{
	set_bit(iParam0, iParam1);
}

int func_25(float fParam0, float fParam1, float fParam2)
{
	float fVar0;
	float fVar1;
	
	fVar1 = (fParam1 - fParam2);
	if (fVar1 < 0f)
	{
		fVar1 = (fVar1 + 360f);
	}
	fVar0 = (fParam1 + fParam2);
	if (fVar0 >= 360f)
	{
		fVar0 = (fVar0 - 360f);
	}
	if (fVar0 < 0f)
	{
		fVar0 = (fVar0 + 360f);
	}
	if (fVar0 > fVar1)
	{
		if (fParam0 < fVar0 && fParam0 > fVar1)
		{
			return 1;
		}
	}
	else if (fParam0 < fVar0 || fParam0 > fVar1)
	{
		return 1;
	}
	return 0;
}

void func_26(var uParam0)
{
	int iVar0;
	
	switch (uParam0->f_13)
	{
		case 4:
		case 8:
		case 6:
			func_31(1);
			invalidate_idle_cam();
			hud_force_weapon_wheel(false);
			display_ammo_this_frame(false);
			hide_hud_component_this_frame(19);
			hide_hud_component_this_frame(2);
			_hud_weapon_wheel_ignore_selection();
			func_30();
			if (get_current_ped_weapon(player_ped_id(), &iVar0, true) && iVar0 != joaat("weapon_unarmed"))
			{
				set_current_ped_weapon(player_ped_id(), joaat("weapon_unarmed"), true);
			}
			disable_control_action(0, 37, true);
			func_28(1);
			func_27();
			break;
	}
}

void func_27()
{
	Global_23011.f_6 = 1;
}

void func_28(bool bParam0)
{
	if (bParam0)
	{
		if (func_29())
		{
			Global_2714762.f_41 = 1;
		}
	}
	else
	{
		Global_2714762.f_41 = 0;
	}
}

bool func_29()
{
	return BitTest(Global_2714762.f_2, 11);
}

void func_30()
{
	set_bit(&Global_8137, 4);
}

void func_31(int iParam0)
{
	if (func_37())
	{
		return;
	}
	if (!Global_20266.f_1 == 1)
	{
		if (func_36(0))
		{
			func_32(iParam0);
		}
		set_bit(&Global_8137, 2);
	}
}

void func_32(int iParam0)
{
	if (func_37())
	{
		return;
	}
	if (Global_20464)
	{
		if (func_35())
		{
			func_34(1, 1);
		}
		else
		{
			func_34(0, 0);
		}
	}
	if (Global_20266.f_1 == 10 || Global_20266.f_1 == 9)
	{
		set_bit(&Global_8137, 16);
	}
	if (is_mobile_phone_call_ongoing())
	{
		stop_scripted_conversation(false);
	}
	Global_21605 = 5;
	if (iParam0 == 1)
	{
		set_bit(&Global_8136, 30);
	}
	else
	{
		clear_bit(&Global_8136, 30);
	}
	if (!func_33())
	{
		Global_20266.f_1 = 3;
	}
}

int func_33()
{
	if (Global_20266.f_1 == 1 || Global_20266.f_1 == 0)
	{
		return 1;
	}
	return 0;
}

void func_34(bool bParam0, bool bParam1)
{
	if (bParam0)
	{
		if (func_36(0))
		{
			Global_20464 = 1;
			if (bParam1)
			{
				get_mobile_phone_position(&Global_20203);
			}
			Global_20194 = { Global_20212[Global_20211 /*3*/] };
			set_mobile_phone_position(Global_20194);
		}
	}
	else if (Global_20464 == 1)
	{
		Global_20464 = 0;
		Global_20194 = { Global_20219[Global_20211 /*3*/] };
		if (bParam1)
		{
			set_mobile_phone_position(Global_20203);
		}
		else
		{
			set_mobile_phone_position(Global_20194);
		}
	}
}

bool func_35()
{
	return BitTest(Global_1958711, 5);
}

int func_36(int iParam0)
{
	if (iParam0 == 1)
	{
		if (Global_20266.f_1 > 3)
		{
			if (BitTest(Global_8136, 14))
			{
				return 1;
			}
			else
			{
				return 0;
			}
		}
		else
		{
			return 0;
		}
	}
	if (_get_number_of_references_of_script_with_name_hash(joaat("cellphone_flashhand")) > 0)
	{
		return 1;
	}
	if (Global_20266.f_1 > 3)
	{
		return 1;
	}
	return 0;
}

bool func_37()
{
	return BitTest(Global_1958711, 19);
}

Vector3 func_38(var uParam0)
{
	struct<3> Var0;
	
	Var0 = { uParam0->f_16[uParam0->f_9 /*14*/].f_3 };
	if (func_11() && uParam0->f_1.f_1 == 0)
	{
		Var0.f_2 = (Var0.f_2 - 4f);
	}
	return Var0;
}

Vector3 func_39(var uParam0)
{
	return uParam0->f_16[uParam0->f_9 /*14*/];
}

void func_40(var uParam0, char* sParam1)
{
	if (uParam0->f_1.f_1 == 1)
	{
		if (func_11())
		{
			switch (uParam0->f_1.f_3)
			{
				case 0:
					StringCopy(sParam1, "IDLE_A", 16);
					break;
				
				case 1:
					StringCopy(sParam1, "IDLE_B", 16);
					break;
				
				case 2:
					StringCopy(sParam1, "IDLE_C", 16);
					break;
				
				case 3:
					StringCopy(sParam1, "ENTER", 16);
					break;
				
				case 5:
					StringCopy(sParam1, "BASE", 16);
					break;
				
				case 4:
					StringCopy(sParam1, "EXIT", 16);
					break;
				
				case 7:
					StringCopy(sParam1, "D_TO_B", 16);
					break;
				
				case 8:
					StringCopy(sParam1, "B_TO_C", 16);
					break;
				
				case 9:
					StringCopy(sParam1, "C_TO_D", 16);
					break;
				
				case 10:
					StringCopy(sParam1, "D_TO_B", 16);
					break;
				
				case 12:
					StringCopy(sParam1, "B_TO_A", 16);
					break;
				
				case 13:
					StringCopy(sParam1, "C_TO_A", 16);
					break;
				
				case 14:
					StringCopy(sParam1, "D_TO_B", 16);
					break;
				
				case 15:
					StringCopy(sParam1, "E_TO_A", 16);
					break;
				
				default:
					StringCopy(sParam1, "invalid_clip", 16);
					break;
			}
		}
		else
		{
			switch (uParam0->f_1.f_3)
			{
				case 0:
					StringCopy(sParam1, "IDLE_A", 16);
					break;
				
				case 1:
					StringCopy(sParam1, "IDLE_B", 16);
					break;
				
				case 2:
					StringCopy(sParam1, "IDLE_C", 16);
					break;
				
				case 3:
					StringCopy(sParam1, "ENTER", 16);
					break;
				
				case 5:
					StringCopy(sParam1, "BASE", 16);
					break;
				
				case 4:
					StringCopy(sParam1, "EXIT", 16);
					break;
				
				case 7:
					StringCopy(sParam1, "A_TO_B", 16);
					break;
				
				case 8:
					StringCopy(sParam1, "B_TO_C", 16);
					break;
				
				case 9:
					StringCopy(sParam1, "A_TO_D", 16);
					break;
				
				case 10:
					StringCopy(sParam1, "D_TO_E", 16);
					break;
				
				case 12:
					StringCopy(sParam1, "B_TO_E", 16);
					break;
				
				case 13:
					StringCopy(sParam1, "C_TO_A", 16);
					break;
				
				case 14:
					StringCopy(sParam1, "D_TO_A", 16);
					break;
				
				case 15:
					StringCopy(sParam1, "E_TO_A", 16);
					break;
				
				default:
					StringCopy(sParam1, "invalid_clip", 16);
					break;
				}
		}
	}
	else
	{
		switch (uParam0->f_1.f_3)
		{
			case 0:
				StringCopy(sParam1, "IDLE_A", 16);
				break;
			
			case 1:
				StringCopy(sParam1, "IDLE_B", 16);
				break;
			
			case 2:
				StringCopy(sParam1, "IDLE_C", 16);
				break;
			
			case 3:
				StringCopy(sParam1, "ENTER", 16);
				break;
			
			case 5:
				StringCopy(sParam1, "BASE", 16);
				break;
			
			case 4:
				StringCopy(sParam1, "EXIT", 16);
				break;
			
			case 7:
				StringCopy(sParam1, "A_TO_B", 16);
				break;
			
			case 8:
				StringCopy(sParam1, "B_TO_C", 16);
				break;
			
			case 9:
				StringCopy(sParam1, "C_TO_D", 16);
				break;
			
			case 10:
				StringCopy(sParam1, "D_TO_E", 16);
				break;
			
			case 12:
				StringCopy(sParam1, "B_TO_E", 16);
				break;
			
			case 13:
				StringCopy(sParam1, "C_TO_A", 16);
				break;
			
			case 14:
				StringCopy(sParam1, "D_TO_A", 16);
				break;
			
			case 15:
				StringCopy(sParam1, "E_TO_A", 16);
				break;
			
			default:
				StringCopy(sParam1, "invalid_clip", 16);
				break;
			}
	}
}

void func_41(var uParam0, char* sParam1)
{
	switch (uParam0->f_1.f_1)
	{
		case 0:
			StringCopy(sParam1, "ANIM@AMB@CLUBHOUSE@SEATING", 64);
			break;
		
		case 1:
			StringCopy(sParam1, "ANIM@AMB@OFFICE@SEATING", 64);
			break;
		
		case 2:
			StringCopy(sParam1, "ANIM@AMB@FACILITY@BRIEFING_ROOM@SEATING", 64);
			break;
	}
	if (func_11())
	{
		StringConCat(sParam1, "@FEMALE", 64);
	}
	else
	{
		StringConCat(sParam1, "@MALE", 64);
	}
	switch (uParam0->f_1.f_1)
	{
		case 1:
			if (func_11())
			{
				switch (uParam0->f_1.f_2)
				{
					case 0:
						StringConCat(sParam1, "@VAR_B@BASE@", 64);
						break;
					
					case 1:
						StringConCat(sParam1, "@VAR_B@BASE@", 64);
						break;
					
					case 2:
						StringConCat(sParam1, "@VAR_C@BASE@", 64);
						break;
					
					case 3:
						StringConCat(sParam1, "@VAR_D@BASE@", 64);
						break;
					
					case 4:
						StringConCat(sParam1, "@VAR_B@BASE@", 64);
						break;
				}
			}
			else
			{
				switch (uParam0->f_1.f_2)
				{
					case 0:
						StringConCat(sParam1, "@VAR_A@BASE@", 64);
						break;
					
					case 1:
						StringConCat(sParam1, "@VAR_B@BASE@", 64);
						break;
					
					case 2:
						StringConCat(sParam1, "@VAR_C@BASE@", 64);
						break;
					
					case 3:
						StringConCat(sParam1, "@VAR_D@BASE@", 64);
						break;
					
					case 4:
						StringConCat(sParam1, "@VAR_E@BASE@", 64);
						break;
					}
			}
			break;
		
		case 0:
			switch (uParam0->f_1.f_2)
			{
				case 0:
					StringConCat(sParam1, "@VAR_A@BASE@", 64);
					break;
				
				case 1:
					StringConCat(sParam1, "@VAR_B@BASE@", 64);
					break;
				
				case 2:
					StringConCat(sParam1, "@VAR_C@BASE@", 64);
					break;
				
				case 3:
					StringConCat(sParam1, "@VAR_D@BASE@", 64);
					break;
				
				case 4:
					StringConCat(sParam1, "@VAR_E@BASE@", 64);
					break;
			}
			break;
		
		case 2:
			switch (uParam0->f_1.f_2)
			{
				case 0:
					StringConCat(sParam1, "@VAR_A@", 64);
					break;
				
				case 1:
					StringConCat(sParam1, "@VAR_B@", 64);
					break;
				
				case 2:
					StringConCat(sParam1, "@VAR_C@", 64);
					break;
				
				case 3:
					StringConCat(sParam1, "@VAR_D@", 64);
					break;
				
				case 4:
					StringConCat(sParam1, "@VAR_E@", 64);
					break;
			}
			break;
	}
}

void func_42(var uParam0)
{
	uParam0->f_1.f_1 = func_44(uParam0->f_16[uParam0->f_9 /*14*/].f_13);
	uParam0->f_1.f_2 = func_43(uParam0);
	uParam0->f_1.f_3 = 3;
	uParam0->f_1.f_4 = 3;
}

int func_43(var uParam0)
{
	int iVar0;
	
	switch (uParam0->f_1.f_1)
	{
		case 2:
			iVar0 = get_random_int_in_range(0, 4);
			if (iVar0 >= 1)
			{
				iVar0++;
			}
			break;
		
		default:
			iVar0 = get_random_int_in_range(0, func_13(uParam0->f_1.f_1));
	}
	if (uParam0->f_1.f_1 == 1)
	{
		if (func_11())
		{
			if (iVar0 == 0 || iVar0 == 4)
			{
				iVar0 = 1;
			}
		}
		else if (iVar0 == 1 || iVar0 == 2)
		{
			iVar0 = 3;
		}
	}
	return iVar0;
}

int func_44(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 1;
		
		case 2:
			return 0;
		
		case 3:
			return 2;
		
		default:
	}
	return 0;
}

void func_45(var uParam0, int iParam1, char* sParam2, int iParam3, char* sParam4, int iParam5, int iParam6)
{
	int iVar0;
	
	if (_get_number_of_references_of_script_with_name_hash(joaat("context_controller")) < 1)
	{
	}
	if (is_player_switch_in_progress())
	{
		if (!*uParam0 == -1)
		{
			func_6(uParam0);
		}
		return;
	}
	if (!*uParam0 == -1)
	{
		return;
	}
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 6)
	{
		if (!Global_43792[iVar0 /*32*/])
		{
			Global_43792[iVar0 /*32*/] = 1;
			Global_43792[iVar0 /*32*/].f_1 = Global_43993;
			Global_43993++;
			Global_43792[iVar0 /*32*/].f_4 = 0;
			Global_43792[iVar0 /*32*/].f_29 = 0;
			Global_43792[iVar0 /*32*/].f_5 = 0;
			Global_43792[iVar0 /*32*/].f_2 = iParam1;
			StringCopy(&(Global_43792[iVar0 /*32*/].f_8), sParam2, 16);
			Global_43792[iVar0 /*32*/].f_6 = iParam3;
			Global_43792[iVar0 /*32*/].f_31 = get_id_of_this_thread();
			Global_43792[iVar0 /*32*/].f_7 = 0;
			Global_43792[iVar0 /*32*/].f_3 = iParam5;
			if (!is_string_null_or_empty(sParam4))
			{
				Global_43792[iVar0 /*32*/].f_12 = 1;
				StringCopy(&(Global_43792[iVar0 /*32*/].f_13), sParam4, 64);
				Global_43792[iVar0 /*32*/].f_30 = iParam6;
			}
			else
			{
				Global_43792[iVar0 /*32*/].f_12 = 0;
				Global_43792[iVar0 /*32*/].f_30 = 0;
			}
			*uParam0 = Global_43792[iVar0 /*32*/].f_1;
			return;
		}
		iVar0++;
	}
}

bool func_46(int iParam0)
{
	return func_47(iParam0) == joaat("weapon_minigun");
}

int func_47(int iParam0)
{
	return Global_1892703[iParam0 /*599*/].f_579;
}

bool func_48(var uParam0)
{
	Stack.Push(!func_23(&(uParam0->f_7), 5));
	Stack.Push(&(uParam0->f_16[uParam0->f_9 /*14*/]));
	Stack.Push(uParam0->f_9);
	Call_Loc(*uParam0);
	return (StackVal || StackVal);
}

int func_49(var uParam0)
{
	var uVar0[10];
	int iVar1;
	
	get_ped_nearby_peds(player_ped_id(), &uVar0, -1);
	iVar1 = 0;
	while (iVar1 < 10)
	{
		if (((does_entity_exist(uVar0[iVar1]) && !is_ped_injured(uVar0[iVar1])) && is_entity_visible(uVar0[iVar1])) && func_56(uVar0[iVar1], &(uParam0->f_16[uParam0->f_9 /*14*/])))
		{
			return 1;
		}
		iVar1++;
	}
	return 0;
}

int func_50(var uParam0)
{
	int iVar0;
	int iVar1;
	
	iVar0 = 0;
	while (iVar0 < network_get_max_num_participants())
	{
		if (network_is_participant_active(int_to_participantindex(iVar0)))
		{
			iVar1 = network_get_player_index(int_to_participantindex(iVar0));
			if (iVar1 != player_id() && func_53(iVar1, 1, 1))
			{
				if (func_56(get_player_ped(iVar1), &(uParam0->f_16[uParam0->f_9 /*14*/])) || func_51(iVar1) == uParam0->f_9)
				{
					return 1;
				}
			}
		}
		iVar0++;
	}
	return 0;
}

int func_51(int iParam0)
{
	if (iParam0 != func_52())
	{
		return Global_1853348[iParam0 /*834*/].f_827;
	}
	return -1;
}

int func_52()
{
	return -1;
}

int func_53(int iParam0, bool bParam1, bool bParam2)
{
	int iVar0;
	
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		if (network_is_player_active(iParam0))
		{
			if (bParam1)
			{
				if (!is_player_playing(iParam0))
				{
					return 0;
				}
			}
			if (bParam2)
			{
				if (iVar0 == Global_2703735.f_3)
				{
					return Global_2703735.f_2;
				}
				else if (Global_2689235[iVar0 /*453*/] != 4)
				{
					return 0;
				}
			}
			return 1;
		}
	}
	return 0;
}

bool func_54(var uParam0)
{
	return uParam0->f_7 & 31 > 0;
}

int func_55(int iParam0, struct<3> Param1, int iParam2)
{
	if (does_entity_exist(iParam0))
	{
		if (iParam2 == 3)
		{
			if (func_25(get_entity_heading(iParam0), (Param1.f_2 - 180f), 55f))
			{
				return 1;
			}
		}
		else if (func_25(get_entity_heading(iParam0), Param1.f_2, 55f))
		{
			return 1;
		}
	}
	return 0;
}

bool func_56(int iParam0, var uParam1)
{
	return is_entity_in_angled_area(iParam0, uParam1->f_6.f_1, uParam1->f_6.f_4, uParam1->f_6, false, true, 0);
}

int func_57(var uParam0)
{
	int iVar0;
	
	iVar0 = 0;
	while (iVar0 < 20)
	{
		if (func_56(player_ped_id(), &(uParam0->f_16[uParam0->f_9 /*14*/])) && func_55(player_ped_id(), uParam0->f_16[uParam0->f_9 /*14*/].f_3, uParam0->f_16[uParam0->f_9 /*14*/].f_13))
		{
			return 1;
		}
		else
		{
			uParam0->f_9 = (uParam0->f_9 + 1 % 15);
		}
		iVar0++;
	}
	return 0;
}

void func_58(int* iParam0, int iParam1)
{
	clear_bit(iParam0, iParam1);
}

void func_59(var uParam0)
{
	if (uParam0->f_13 == 6)
	{
	}
	else if (uParam0->f_13 > 4)
	{
		_disable_cam_collision_for_entity(player_ped_id());
		set_ped_capsule(player_ped_id(), func_60());
		uParam0->f_12 = get_frame_count();
	}
	else if ((get_frame_count() - uParam0->f_12) < 5)
	{
		set_ped_capsule(player_ped_id(), func_60());
	}
}

float func_60()
{
	return 0.13f;
}

void func_61(var uParam0)
{
	func_62(&(uParam0->f_7), 4, func_63());
	func_62(&(uParam0->f_7), 1, is_entity_dead(player_ped_id(), false));
	func_62(&(uParam0->f_7), 3, is_ped_in_any_vehicle(player_ped_id(), true));
	func_62(&(uParam0->f_7), 0, func_36(0));
	func_62(&(uParam0->f_7), 2, is_ped_running(player_ped_id()));
}

void func_62(int* iParam0, int iParam1, bool bParam2)
{
	int iVar0;
	
	iVar0 = iParam1;
	if (bParam2)
	{
		set_bit(iParam0, iVar0);
	}
	else
	{
		clear_bit(iParam0, iVar0);
	}
}

bool func_63()
{
	return Global_75485;
}

int func_64()
{
	if (is_screen_faded_out())
	{
		return 1;
	}
	if (func_75(player_id()))
	{
		return 1;
	}
	if (func_74(player_id()))
	{
		return 1;
	}
	if (get_distance_between_coords(func_73(player_id()), Global_1946250.f_1011[154 /*3*/], true) >= 150f)
	{
		return 1;
	}
	if (func_65())
	{
		return 1;
	}
	return 0;
}

int func_65()
{
	if (Global_1575033 == 0)
	{
		if (!network_is_game_in_progress())
		{
			return 1;
		}
	}
	if (func_72())
	{
		return 1;
	}
	if (Global_2727795)
	{
		return 1;
	}
	if (func_71())
	{
		return 1;
	}
	if (func_70(159))
	{
		if (!func_69())
		{
			return 1;
		}
	}
	if (func_70(157))
	{
		return 1;
	}
	if (!network_is_signed_online())
	{
		return 1;
	}
	if (func_66() != 0)
	{
		if (_get_number_of_references_of_script_with_name_hash(func_66()) == 0)
		{
			return 1;
		}
	}
	return 0;
}

int func_66()
{
	switch (func_68())
	{
		case 0:
			return func_67();
			break;
		
		case 2:
			return joaat("creator");
			break;
	}
	return 0;
}

int func_67()
{
	switch (Global_2727899)
	{
		case 0:
			return joaat("freemode");
		
		default:
	}
	return joaat("freemode");
}

int func_68()
{
	return Global_31959;
}

bool func_69()
{
	return Global_2714762.f_698;
}

int func_70(int iParam0)
{
	if (get_event_exists(1, iParam0))
	{
		return 1;
	}
	return 0;
}

bool func_71()
{
	return Global_2725403;
}

bool func_72()
{
	return Global_2714762.f_693;
}

Vector3 func_73(int iParam0)
{
	return get_entity_coords(get_player_ped(iParam0), false);
}

int func_74(int iParam0)
{
	if (iParam0 != func_52())
	{
		if (func_53(iParam0, 1, 1))
		{
			return Global_2689235[iParam0 /*453*/].f_318.f_6 != -1;
		}
		else if ((Global_1575058 && iParam0 == player_id()) && func_53(iParam0, 1, 0))
		{
			return Global_2689235[iParam0 /*453*/].f_318.f_6 != -1;
		}
	}
	return 0;
}

int func_75(int iParam0)
{
	if (iParam0 != func_52() && func_53(iParam0, 1, 1))
	{
		return BitTest(Global_2689235[iParam0 /*453*/].f_318, 3);
	}
	return 0;
}

void func_76()
{
	func_78(&uLocal_93);
	func_77();
}

void func_77()
{
	terminate_this_thread();
}

void func_78(var uParam0)
{
	int iVar0;
	
	func_6(&(uParam0->f_6));
	func_4(uParam0, 1);
	if (uParam0->f_13 > 2)
	{
		func_3(-1);
		iVar0 = network_get_local_scene_from_network_id(uParam0->f_8);
		if (is_synchronized_scene_running(iVar0))
		{
			network_stop_synchronised_scene(uParam0->f_8);
		}
	}
}

void func_79()
{
	network_set_this_script_is_network_script(32, false, -1);
	func_89(0, -1, 0);
	func_88();
	func_80();
	if (network_is_game_in_progress())
	{
	}
	else
	{
		func_76();
	}
}

void func_80()
{
	func_81(0, &uLocal_93, 0, 2);
	func_81(1, &uLocal_93, 1, 1);
	func_81(2, &uLocal_93, 2, 1);
	func_81(3, &uLocal_93, 3, 2);
	func_81(4, &uLocal_93, 4, 1);
	func_81(5, &uLocal_93, 5, 2);
	func_81(6, &uLocal_93, 6, 2);
	func_81(7, &uLocal_93, 7, 2);
	func_81(8, &uLocal_93, 8, 2);
	func_81(9, &uLocal_93, 9, 1);
	func_81(10, &uLocal_93, 10, 2);
	func_81(11, &uLocal_93, 11, 1);
	func_81(12, &uLocal_93, 12, 1);
	func_81(13, &uLocal_93, 13, 1);
	func_81(14, &uLocal_93, 14, 1);
}

void func_81(int iParam0, var uParam1, int iParam2, int iParam3)
{
	struct<3> Var0;
	float fVar1;
	
	func_87(iParam0, &Var0, &fVar1);
	uParam1->f_16[iParam2 /*14*/].f_13 = iParam3;
	uParam1->f_16[iParam2 /*14*/] = { Var0 };
	uParam1->f_16[iParam2 /*14*/].f_3 = { 0f, 0f, fVar1 };
	uParam1->f_16[iParam2 /*14*/].f_6.f_1 = { func_84(func_86(), func_85(iParam3, fVar1)) + Var0 };
	uParam1->f_16[iParam2 /*14*/].f_6.f_4 = { func_84(func_83(), func_85(iParam3, fVar1)) + Var0 };
	uParam1->f_16[iParam2 /*14*/].f_6 = func_82();
}

float func_82()
{
	return 1.25f;
}

Vector3 func_83()
{
	return 0.5f, -0.5f, 2f;
}

Vector3 func_84(struct<3> Param0, float fParam1)
{
	struct<3> Var0;
	float fVar1;
	float fVar2;
	
	fVar1 = sin(fParam1);
	fVar2 = cos(fParam1);
	Var0.f_0 = ((Param0.f_0 * fVar2) - (Param0.f_1 * fVar1));
	Var0.f_1 = ((Param0.f_0 * fVar1) + (Param0.f_1 * fVar2));
	Var0.f_2 = Param0.f_2;
	return Var0;
}

float func_85(int iParam0, float fParam1)
{
	switch (iParam0)
	{
		case 1:
		case 2:
			return fParam1;
		
		case 3:
			return (fParam1 - 180f);
		
		default:
	}
	return fParam1;
}

Vector3 func_86()
{
	return -0.5f, -0.5f, 0f;
}

void func_87(int iParam0, var uParam1, var uParam2)
{
	switch (iParam0)
	{
		case 0:
			*uParam1 = { 752.663f, -1783.328f, 48.3f };
			*uParam2 = -37.8f;
			break;
		
		case 1:
			*uParam1 = { 750.85f, -1782.746f, 48.3f };
			*uParam2 = 11.88f;
			break;
		
		case 2:
			*uParam1 = { 750f, -1786.55f, 48.3f };
			*uParam2 = 149.04f;
			break;
		
		case 3:
			*uParam1 = { 778.8f, -1831.568f, 48.3f };
			*uParam2 = -172.44f;
			break;
		
		case 4:
			*uParam1 = { 780.289f, -1827.899f, 48.3f };
			*uParam2 = -30.24f;
			break;
		
		case 5:
			*uParam1 = { 778.538f, -1827.125f, 48.3f };
			*uParam2 = 0f;
			break;
		
		case 6:
			*uParam1 = { 777.238f, -1830.767f, 48.3f };
			*uParam2 = 142.2f;
			break;
		
		case 7:
			*uParam1 = { 768.375f, -1863.114f, 48.3f };
			*uParam2 = 9f;
			break;
		
		case 8:
			*uParam1 = { 769.575f, -1864.481f, 48.3f };
			*uParam2 = -90f;
			break;
		
		case 9:
			*uParam1 = { 768.863f, -1866.382f, 48.3f };
			*uParam2 = -145f;
			break;
		
		case 10:
			*uParam1 = { 747.361f, -1769.411f, 52.311f };
			*uParam2 = -163.44f;
			break;
		
		case 11:
			*uParam1 = { 748.837f, -1767.993f, 52.311f };
			*uParam2 = -128.16f;
			break;
		
		case 12:
			*uParam1 = { 748.061f, -1766.24f, 52.311f };
			*uParam2 = -9.72f;
			break;
		
		case 13:
			*uParam1 = { 765.861f, -1776.777f, 52.311f };
			*uParam2 = -151.56f;
			break;
		
		case 14:
			*uParam1 = { 764.161f, -1777.399f, 52.311f };
			*uParam2 = 159.84f;
			break;
	}
}

void func_88()
{
}

int func_89(int iParam0, int iParam1, bool bParam2)
{
	int iVar0;
	
	iVar0 = network_get_script_status();
	while (iVar0 != 2)
	{
		if (((iVar0 == 3 || iVar0 == 4) || iVar0 == 5) || iVar0 == 6)
		{
			if (!bParam2)
			{
				func_77();
			}
			else
			{
				return 0;
			}
		}
		if (!func_90(0))
		{
			if (iParam0 == 0)
			{
				if (!network_is_game_in_progress())
				{
					if (!bParam2)
					{
						func_77();
					}
					else
					{
						return 0;
					}
				}
				if (func_72())
				{
					if (!bParam2)
					{
						func_77();
					}
					else
					{
						return 0;
					}
				}
				if (func_70(157))
				{
					if (!bParam2)
					{
						func_77();
					}
					else
					{
						return 0;
					}
				}
			}
			else if (!network_is_in_session())
			{
				if (!bParam2)
				{
					func_77();
				}
				else
				{
					return 0;
				}
			}
		}
		wait(0);
		iVar0 = network_get_script_status();
	}
	if (iParam1 > -1)
	{
		Global_1574666 = iVar0;
	}
	if (iParam0 == 0)
	{
		if (!network_is_game_in_progress())
		{
			if (!bParam2)
			{
				func_77();
			}
			else
			{
				return 0;
			}
		}
	}
	else if (!network_is_in_session())
	{
		if (!bParam2)
		{
			func_77();
		}
		else
		{
			return 0;
		}
	}
	return 1;
}

bool func_90(bool bParam0)
{
	if (bParam0)
	{
	}
	return Global_1575033;
}

