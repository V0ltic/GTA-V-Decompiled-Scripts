...
void __EntryFunction__()
{
	...
	Local_274 = { 0f, 0f, 0f };
	Local_275 = { 0f, 0f, 0f };
	iLocal_280 = get_random_int_in_range(0, 4);
	func_76();
	...
	if (does_entity_exist(iScriptParam_286))
	{
		iLocal_41 = iScriptParam_286;
		freeze_entity_position(iLocal_41, true);
		iLocal_40 = get_entity_model(iLocal_41);
	}
	func_68(&uLocal_108, 2, player_ped_id(), "TREVOR", 0, 1);
	while (true)
	{
		wait(0);
		if (does_entity_exist(iLocal_41))
		{
			if (((((is_object_within_brain_activation_range(iLocal_41) && !is_entity_dead(iLocal_41, false)) && !is_ped_injured(player_ped_id())) && func_63()) && func_62(0)) && !is_player_switch_in_progress())
			{
				switch (iLocal_276)
				{
					case 0:
...
void func_1()
{
	...
	{
		case 0:
			if (func_52(Var0, Var1) && is_ped_heading_towards_position(player_ped_id(), get_entity_coords(iLocal_41, true), 90f))
			{
				func_76();
			...
			if (func_49(1, 0, 1))
			{
				if (func_52(Var0, Var1))
				{
					if ((!is_player_free_aiming(player_id()) && !is_player_targetting_anything(player_id())) && is_control_pressed(2, 51))
					{
						...
						clear_area_of_projectiles(Var0, 3f, 0);
		...
		case 2:
			...
			iLocal_278 = create_synchronized_scene(func_37(), func_36(), 2);
			task_synchronized_scene(player_ped_id(), iLocal_278, func_35(), func_34(iLocal_280), 8f, -2f, 0, 0, 1000f, 0);
			play_synchronized_entity_anim(iLocal_41, iLocal_278, func_33(iLocal_280), func_35(), 8f, -2f, 0, 1000f);
			if (is_synchronized_scene_hold_last_frame(iLocal_278))
			{
				set_synchronized_scene_hold_last_frame(iLocal_278, false);
			}
			if (get_follow_ped_cam_view_mode() != 4)
			{
				iLocal_279 = create_cam("DEFAULT_ANIMATED_CAMERA", true);
				play_synchronized_cam_anim(iLocal_279, iLocal_278, func_30(iLocal_280), func_35());
			...
			iLocal_283 = 0;
			iLocal_284 = (get_game_timer() + func_29());
			iLocal_277 = 3;
			break;
		...
		case 3:
			...
			hide_hud_and_radar_this_frame();
			disable_control_action(1, 0, true);
			if (is_synchronized_scene_running(iLocal_278))
			{
				func_27();
				if (iLocal_281)
				{
					fVar2 = get_synchronized_scene_phase(iLocal_278);
					if (find_anim_event_phase(func_35(), func_34(iLocal_280), "WalkInterruptible", &fVar3, &fVar4))
					{
				...
				if (!iLocal_283 && get_game_timer() > iLocal_284)
				{
		...
		case 5:
			...
			if (stat_get_int(joaat("num_sh_mr_jam_used"), &iLocal_285, -1))
			{
				stat_set_int(joaat("num_sh_mr_jam_used"), iLocal_285 + 1, true);
			}
			...
			iLocal_281 = 0;
			if (get_entity_health(player_ped_id()) < (get_ped_max_health(player_ped_id()) - 10))
			{
				set_entity_health(player_ped_id(), get_entity_health(player_ped_id()) + 10, 0);
			}
...
char* func_31()
{
	...
	fVar2 = get_distance_between_coords(Var1, Local_273[0 /*3*/], true);
	fVar3 = get_distance_between_coords(Var1, Local_273[1 /*3*/], true);
	if (fVar2 < fVar3)
	{
...
void func_40(bool bParam0, bool bParam1, int iParam2, bool bParam3, bool bParam4, bool bParam5, bool bParam6)
{
	...
	else
	{
		...
		{
			if (((((!is_ped_injured(player_ped_id()) && !func_45(player_id())) && !func_42(player_id(), 0)) && !func_41()) && !bParam4) && !bParam5)
			{
				set_entity_invincible(player_ped_id(), false);
			}
		}
		else if (((!is_ped_injured(player_ped_id()) && !func_45(player_id())) && !bParam4) && !bParam5)
		{
			set_entity_invincible(player_ped_id(), false);
		}
		Global_78317 = 0;
...
int func_52(struct<3> Param0, float fParam1, var uParam2, var uParam3)
{
	if (is_player_playing(player_id()))
	{
		if ((!is_ped_in_any_vehicle(player_ped_id(), false) && !is_entity_on_fire(player_ped_id())) && is_player_control_on(player_id()))
		{
			...
			{
				if ((((is_gameplay_cam_rendering() && !is_cutscene_playing()) && !is_cinematic_cam_rendering()) && !func_56(0)) && !func_55())
				{
					if ((Global_97369 && Global_97370) || func_54())
					{
						return 0;
					}
					if (func_53(get_entity_coords(player_ped_id(), true), Param0, fParam1, 0))
					{
						return 1;
					}
				}
...
int func_58()
{
	if (request_ambient_audio_bank(func_59(), false, -1))
	{
		return 1;
	}
	return 0;
...
int func_61()
{
	...
	while (!has_anim_dict_loaded(sVar0))
	{
		wait(0);
	}
	return 1;
...
int func_62(int iParam0)
{
	...
	if (is_player_playing(player_id()))
	{
		...
		{
			if (func_53(get_entity_coords(iLocal_41, false), func_57(), 5f, 0))
			{
				return 1;
			}
		}
		...
		else if (get_interior_from_entity(player_ped_id()) == get_interior_from_entity(iParam0))
		{
			return 1;
		}
	}
...
void func_75()
{
	if (is_player_playing(player_id()))
	{
		if ((!is_player_control_on(player_id()) && func_49(1, 0, 1)) && iLocal_36)
		{
			func_40(0, 1, 1, 0, 0, 0, 0);
			if (!is_ped_injured(player_ped_id()))
			{
