		...
		switch (iLocal_21)
		{
			...
			case 1:
				...
				while (iLocal_19 < Global_22724[1 /*42*/].f_32[1])
				{
					...
					{
						if (does_entity_exist(uLocal_20[iLocal_19]))
						{
							if (is_ped_injured(uLocal_20[iLocal_19]))
							{
								if (has_entity_been_damaged_by_entity(uLocal_20[iLocal_19], player_ped_id(), true))
								{
									...
									if (is_entity_dead(uLocal_20[iLocal_19], false))
									{
										if (!Global_32014[iLocal_19])
										{
											if (cell_cam_is_char_visible_no_face_check(uLocal_20[iLocal_19]) && func_1())
											{
...
void func_2(char* sParam0, int iParam1, int iParam2, int iParam3, int iParam4)
{
	...
	end_text_command_print(iParam3, false);
}
...
void func_3(char* sParam0, char* sParam1, int iParam2, int iParam3)
{
	...
	end_text_command_print(iParam2, false);
}
...
{
	if (_get_number_of_references_of_script_with_name_hash(joaat("appsidetask")) > 0)
	{
		return 0;
	}
	if (func_8() == 0)
