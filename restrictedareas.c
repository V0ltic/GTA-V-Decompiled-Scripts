					...
					else
					{
						if (iLocal_42)
						{
							...
							while (iLocal_44 < 8)
							{
								...
								{
									if (func_38(get_entity_coords(player_ped_id(), false), iLocal_44 + 1, 0, 0))
									{
						...
						if (timera() > 125)
						{
							...
							if (func_35(bLocal_37, get_entity_coords(player_ped_id(), false)))
							{
								...
								if (func_34(bLocal_37))
								{
									...
									if (_get_number_of_references_of_script_with_name_hash(get_hash_key(&Local_48)) == 0)
									{
								...
								else if (!Global_32174[bLocal_37])
								{
									if (is_ped_in_flying_vehicle(player_ped_id()))
									{
										...
										if (!bLocal_40)
										{
											if (func_38(get_entity_coords(player_ped_id(), false), bLocal_37, 0, 0))
											{
												if ((get_game_timer() - iLocal_49[iLocal_46]) > 8000)
												{
													func_24();
												}
												else if (is_projectile_in_area(get_entity_coords(player_ped_id(), false) + Vector(200f, 200f, 200f), get_entity_coords(player_ped_id(), false) - Vector(200f, 200f, 200f), true))
												{
													func_24();
												}
												else if (bLocal_37 != 4)
											...
											{
												iLocal_49[iLocal_46] = get_game_timer();
											}
										}
									}
									else
									{
										...
										if (!bLocal_40)
										{
											if (func_38(get_entity_coords(player_ped_id(), false), bLocal_37, 0, 0))
											{
												if (!bLocal_37 == 4 || (get_game_timer() - iLocal_49[iLocal_46]) > 8000)
												{
													func_24();
												}
											}
...
void func_24()
{
	switch (iLocal_46)
	{
		case 0:
			iLocal_47 = 2;
			if (get_player_wanted_level(player_id()) < iLocal_47)
			{
			}
			break;
		
		case 1:
			if (is_ped_in_flying_vehicle(player_ped_id()) && is_projectile_in_area(get_entity_coords(player_ped_id(), false) + Vector(200f, 200f, 200f), get_entity_coords(player_ped_id(), false) - Vector(200f, 200f, 200f), true))
			{
				iLocal_47 = 5;
			}
			else
	...
	if (get_player_wanted_level(player_id()) < iLocal_47)
	{
		set_player_wanted_level(player_id(), iLocal_47, false);
