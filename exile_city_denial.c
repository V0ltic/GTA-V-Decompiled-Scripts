...
void __EntryFunction__()
{
	...
	iLocal_28 = -1;
	bLocal_32 = true;
	if (_get_number_of_references_of_script_with_name_hash(joaat("exile_city_denial")) > 1)
	{
		Global_43017 = 0;
	...
	while (true)
	{
			...
			if (bLocal_29 && !bVar4)
			{
				...
				if (bVar5)
				{
					...
					if (iVar1 == 0 && !bVar4)
					{
						...
						{
							if (_get_num_dispatched_units_for_player(11) < 256)
							{
								if (does_entity_exist(player_ped_id()))
								{
									if (!is_ped_injured(player_ped_id()))
									{
										create_incident_with_entity(11, player_ped_id(), iVar0, 0f, &iLocal_33, 0, 0);
									}
								}
							}
		...
		if (bLocal_29)
		{
			if (!is_incident_valid(iLocal_33))
			{
				if (_get_num_dispatched_units_for_player(11) < 256)
				{
					if (does_entity_exist(player_ped_id()))
		...
		if (bLocal_29 && timerb() > 60000)
		{
			...
			if (iVar0 < 8)
			{
				if (is_incident_valid(iLocal_33))
				{
					set_incident_requested_units(iLocal_33, 11, iVar0);
				}
				else if (_get_num_dispatched_units_for_player(11) < 256)
				{
					if (does_entity_exist(player_ped_id()))
					{
						if (!is_ped_injured(player_ped_id()))
...
void func_5(int iParam0, bool bParam1)
{
	...
	if (bParam1)
	{
		set_bit(&(Global_113386.f_7229.f_17[iParam0]), false);
	}
	else
	{
		clear_bit(&(Global_113386.f_7229.f_17[iParam0]), false);
	}
	disable_police_restart(Global_95580[iParam0 /*9*/].f_8, !bParam1);
...
void func_7(int iParam0, bool bParam1)
{
	...
	if (bParam1)
	{
		set_bit(&(Global_113386.f_7229.f_11[iParam0]), false);
	}
	else
	{
		clear_bit(&(Global_113386.f_7229.f_11[iParam0]), false);
	}
	disable_hospital_restart(Global_95644[iParam0 /*9*/].f_8, !bParam1);
