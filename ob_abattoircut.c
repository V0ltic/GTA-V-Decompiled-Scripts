...
void __EntryFunction__()
{
	...
	while (true)
	{
		...
		if (does_entity_exist(iScriptParam_3))
		{
			if (is_object_within_brain_activation_range(iScriptParam_3) && _get_number_of_references_of_script_with_name_hash(joaat("michael2")) > 0)
			{
				switch (iLocal_2)
				{
					case 0:
						if (does_entity_have_drawable(iScriptParam_3))
						{
							...
							{
								play_entity_anim(iScriptParam_3, "beefsplitter_loop", "MISSMIC2", 1f, true, false, false, 0f, 0);
								iLocal_2 = 1;
							}
						}
						break;
